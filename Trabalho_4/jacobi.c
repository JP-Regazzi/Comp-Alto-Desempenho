...
int run(double *A, double *b, double *x, double *xtmp)
{
  ...
  do
  {
    ...
    #pragma omp parallel for shared(A, b, x, xtmp) private(row, col, dot) num_threads(THREADS_NUMBER)
    for (row = 0; row < N; row++)
    {
      dot = 0.0;
      for (col = 0; col < N; col++)
    ...
    sqdiff = 0.0;
    #pragma omp parallel for shared(x, xtmp) private(row, diff) reduction(+: sqdiff) num_threads(THREADS_NUMBER)
    for (row = 0; row < N; row++)
    {
      diff    = xtmp[row] - x[row];
      sqdiff += diff * diff;
...
void parse_arguments(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    {
      if (++i >= argc || (THREADS_NUMBER = parse_int(argv[i])) < 0)
      {
